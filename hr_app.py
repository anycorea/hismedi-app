    ...
    components.html(
        ...
        (function(){
          ...
          ["연도","연도(현황판)"].forEach(t=>{
            ...
            if(!lab) return;
            lab.style.cssText="background:#FDE68A;padding:2px 8px;border-radius:8px;font-weight:700;color:#111";
            const inp=lab.closest('div[data-testid^="stNumberInput"]')?.querySelector('input');
            if(inp){ inp.style.boxShadow="0 0 0 3px rgba(245,158,11,.35)"; inp.style.borderColor="#F59E0B"; }
          });
        })();
        </script>
        """,
