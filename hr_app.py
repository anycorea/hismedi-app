...
def _sort_by_sabun(df):
    if "사번" not in df.columns or df.empty:
        return df
...
def tab_staff(emp_df: pd.DataFrame):
    ...
    opt = view.copy()
    opt["표시"] = opt.apply(lambda r: f"{r.get('사번','')} - {r.get('이름','')}", axis=1)
    pick = st.selectbox("대상자", ["(선택)"] + opt.get("표시", pd.Series(dtype=str)).tolist(), index=0, key="staff_pick")
    if pick != "(선택)":
        sabun = pick.split(" - ", 1)[0]
        c2 = st.columns([1,1,2])
        with c2[0]:
            st.markdown("**PIN 재발급**")
            length = st.number_input("자릿수", min_value=4, max_value=8, value=6, step=1, key="pin_len_staff")
            if st.button("재발급", use_container_width=True, key="pin_reissue_staff"):
                if not am_admin:
                    st.error("Master 전용"); st.stop()
                try:
                    rep = reissue_pin_inline(sabun, int(length))
                    st.success(f"PIN_No: {rep['PIN_No']}", icon="✅")
                except Exception as e:
                    st.exception(e)
        with c2[1]:
            st.markdown("**부서 이동**")
            nd1 = st.text_input("새 부서1", "", key="staff_nd1")
            nd2 = st.text_input("새 부서2", "", key="staff_nd2")
            sdt = st.date_input("발령일", datetime.now(tz=tz_kst()).date(), key="staff_tr_date")
            if st.button("이동 반영", use_container_width=True, key="staff_tr_apply"):
                if not am_admin:
                    st.error("Master 전용"); st.stop()
                if not (str(nd1).strip() or str(nd2).strip()):
                    st.error("부서1/부서2 중 하나는 입력 필요"); st.stop()
                try:
                    rep = dept_transfer_inline(sabun, str(nd1).strip(), str(nd2).strip(), sdt)
                    st.success(f"{rep['부서1']} / {rep['부서2']} ({rep['발령일']}) 반영", icon="✅")
                except Exception as e:
                    st.exception(e)
...
def _has_competency_access(emp_df: pd.DataFrame, sabun: str) -> bool:
    """관리자 또는 evaluator 범위가 1명 이상인 경우에만 True."""
    try:
...
def tab_competency(emp_df: pd.DataFrame):
    ...
    if "재직여부" in df.columns:
        df = df[df["재직여부"] == True]
    if evaluable and not is_admin(me_sabun):
        df = df[df["사번"].isin(evaluable)]  # 관리자는 전체

    if df.empty:
        # 권한이 있더라도 현재 연도에 대상이 없을 수 있음
        st.info("현재 평가 권한 범위 내 대상이 없습니다.", icon="ℹ️")
        return

    # 해당 연도의 JD 중 '직무개요'가 채워진 인원만
    def _sabuns_with_jd_overview(target_year: int) -> set[str]:
        ...
        jj = jdf.copy()
        try:
            jj["연도"] = jj["연도"].astype(int)
        except Exception:
            return set()
        jj = jj[jj["연도"] == int(target_year)]
        if jj.empty:
            return set()
        jj["직무개요"] = jj["직무개요"].astype(str).fillna("")
        # 최신 버전 + 개요 비어있지 않은 행
        if "버전" in jj.columns:
            try:
                jj["버전"] = pd.to_numeric(jj["버전"], errors="coerce").fillna(0)
            except Exception:
                pass
            jj = jj.sort_values(["사번","버전"])
        else:
            jj = jj.reset_index(drop=True)
        jj = jj[jj["직무개요"].str.strip() != ""]
        jj = jj.drop_duplicates("사번", keep="last")
        return set(jj["사번"].astype(str).tolist())

    jd_ok = _sabuns_with_jd_overview(int(year))
    df = df[df["사번"].isin(jd_ok)]

    if df.empty:
        st.info("해당 연도에 '직무개요'가 등록된 평가 대상이 없습니다.", icon="ℹ️")
        return
