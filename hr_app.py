...
# =============================================================
# [A-Plan Full Integration Patch] 7 Tables / Supabase-only CRUD
# - Google Sheets 경로 완전 차단: read/write 모두 Supabase로 우회
# - 환경변수: SUPABASE_URL / SUPABASE_ANON_KEY (또는 SUPABASE_SERVICE_ROLE_KEY)
# =============================================================
try:
...
# Shim for APIError (in case existing code refers to it)
class APIError(Exception):
...
_AP_WRITE_MAP = {
    ...
    "권한": ("acl", ["ID"]),
    "acl": ("acl", ["ID"]),
    "인사평가": ("eval_latest", ["ID"]),  # 필요 시 조합키로 교체 가능
...
# Explicitly disable batch/queue writers for Sheets
def gs_enqueue_range(*args, **kwargs):
