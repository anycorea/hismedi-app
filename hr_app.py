    ...
    try:
        ...
        try:
            if st.secrets.get("app", {}).get("DEBUG", False):
                st.exception(e)  # 개발 모드에서만 상세 스택 노출
        except Exception:
            pass
        return None
...
def tab_competency(emp_df: pd.DataFrame):
    """직무능력평가: JD 유무와 무관하게 권한 내 전 직원을 표시하고, 선택 후 평가 가능."""
    ...
    with colY[0]:
        year = st.number_input("평가 연도", min_value=2000, max_value=2100,
                               value=int(this_year), step=1, key="cmpS_year")
    with colY[1]:
        st.caption("※ JD(직무기술서) 유무와 관계없이 평가 가능합니다.")
    ...
    if "사번" not in df.columns:
        ...
        return
    df = df[df["사번"].astype(str).str.strip() != ""].copy()
    ...
    # 표 렌더 (체크박스 단일 선택; 여러 명 체크하면 마지막 1명만 적용)
    ...
    # 최종 대상자 결정: 표 체크가 있으면 우선, 없으면 셀렉트박스 값
    ...
    with colG[3]:
        try:
            eval_date = st.date_input("평가일자", datetime.now(tz=tz_kst()).date(),
                                      key="cmpS_date").strftime("%Y-%m-%d")
        except Exception:
            eval_date = st.date_input("평가일자", datetime.now().date(),
                                      key="cmpS_date").strftime("%Y-%m-%d")
    ...
    st.markdown("### 내 제출 현황")
    try:
        # 프로젝트 내 구현된 함수명에 맞추세요. (없으면 except로 넘어감)
