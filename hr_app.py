...
def upsert_eval_response(emp_df: pd.DataFrame, year: int, eval_type: str,
    ...
    now=kst_now_str()
    values = _ws_values(ws, key=f"{ws.title}:{_year}:{_rev}"); cY=hmap.get("연도"); cT=hmap.get("평가유형"); cTS=hmap.get("평가대상사번"); cES=hmap.get("평가자사번")
    row_idx=0
    for i in range(2, len(values)+1):
        r=values[i-1]
        try:
...
def upsert_comp_simple_response(emp_df: pd.DataFrame, year:int, target_sabun:str,
    ...
    now=kst_now_str()
    values = _ws_values(ws, key=f"{ws.title}:{_year}:{_rev}"); cY=hmap.get("연도"); cTS=hmap.get("평가대상사번"); cES=hmap.get("평가자사번")
    row_idx=0
    for i in range(2, len(values)+1):
        r=values[i-1]
        try:
